...
public class Scrabble {
	...
	public static int wordScore(String word) {
		...
		for (char c : word.toCharArray()) {
			result1 += SCRABBLE_LETTER_VALUES[c - 'a'];
		}
		result1 *= word.length();
	...
	public static String createHand() {
		...
		for (int i = 0; i < HAND_SIZE - 2; i++) {
			hand += (char) ('a' + r.nextInt(26));
		}
		int aIndex = r.nextInt(hand.length() + 1);
		hand = hand.substring(0, aIndex) + "a" + hand.substring(aIndex);
		int eIndex = r.nextInt(hand.length() + 1);
	...
	public static void playHand(String hand) {
		int score = 0; // Total score for this hand
		In in = new In();
